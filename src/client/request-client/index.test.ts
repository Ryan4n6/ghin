    ...
    let mockCache: InMemoryCacheClient
           ...
           describe('constructor', () => {
                 it('should create instance with valid config', () => {
                        ...
                        it('should create instance with only a token (no username/password)', () => {
                                const { jwtDecode } = require('jwt-decode')
                                jwtDecode.mockReturnValue({ exp: Math.floor(Date.now() / 1000) + 3600 })

                                 const tokenClient = new RequestClient({ token: 'my-pre-issued-token' })
                                expect(tokenClient).toBeInstanceOf(RequestClient)
                        })

                        it('should throw when neither token nor username+password is provided', () => {
                                expect(() => {
                                          new RequestClient({} as unknown as { username: string; password: string })
                                }).toThrow('Invalid RequestClientConfig')
                        })
           })

           describe('pre-issued token', () => {
                 it('should use the pre-issued token directly without making login requests', async () => {
                         const { jwtDecode } = await import('jwt-decode')
                         vi.mocked(jwtDecode).mockReturnValue({ exp: Math.floor(Date.now() / 1000) + 3600 })

                          const validToken = 'pre-issued-valid-token'
                         const tokenClient = new RequestClient({ token: validToken, cache: mockCache })
                         ...
                         vi.mocked(fetch).mockResolvedValueOnce({
                          ...
                          const result = await tokenClient.fetch({ entity: 'golfer', schema })

                          expect(result.isOk()).toBe(true)
                         // Only 1 fetch call (the API call) — no session or login requests
                          expect(fetch).toHaveBeenCalledTimes(1)
                 })

                        it('should return AuthenticationError when the pre-issued token is expired', async () => {
                                const { jwtDecode } = await import('jwt-decode')
                                // Return an expired token (exp in the past)
                                 vi.mocked(jwtDecode).mockReturnValue({ exp: Math.floor(Date.now() / 1000) - 60 })

                                 const expiredToken = 'expired-pre-issued-token'
                                const tokenClient = new RequestClient({ token: expiredToken, cache: mockCache })
                                ...
                                // No fetch calls at all — error before any network request
                                 expect(fetch).not.toHaveBeenCalled()
                        })

                        it('should pass the pre-issued token as Bearer authorization header', async () => {
                                const { jwtDecode } = await import('jwt-decode')
                                vi.mocked(jwtDecode).mockReturnValue({ exp: Math.floor(Date.now() / 1000) + 3600 })

                                 const validToken = 'my-bearer-token-value'
                                const tokenClient = new RequestClient({ token: validToken, cache: mockCache })

                                 const mockApiResponse = { golfers: [] }
                                         vi.mocked(fetch).mockResolvedValueOnce({
                                                   ok: true,
                                                   json: async () => mockApiResponse,
                                         } as Response)
                                 ...
                                 await tokenClient.fetch({ entity: 'golfers_search', schema })
           ...
           describe('fetch', () => {
                 it('should make authenticated request with valid token', async () => {
                         ...
                         const mockLoginResponse = { golfer_user: { golfer_user_token: 'access-token' } }
                         const mockApiResponse = { data: 'test-data' }
                        ...
                        it('should handle API errors', async () => {
                                ...
                                vi.mocked(jwtDecode).mockReturnValue({
                                 ...
                                 const mockSessionResponse = { authToken: { token: 'session-token', expiresIn: '3600s' } }
                                const mockLoginResponse = { golfer_user: { golfer_user_token: 'access-token' } }
